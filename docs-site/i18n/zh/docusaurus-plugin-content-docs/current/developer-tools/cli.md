...
它构建在 `plebbit-js` 之上，并由 [5chan](/apps/5chan/) 和 [Seedit](/apps/seedit/) 用于社区创建和节点管理。

## 安装

预构建的二进制文件可用于 Windows、macOS 和 Linux。从 GitHub 下载适合您平台的最新版本：
...
首次启动时，守护进程输出指向 **WebUI** 的链接，这是一个基于浏览器的图形界面，用于管理节点、社区和设置。如果您更喜欢 GUI 而不是终端命令，这非常有用。
