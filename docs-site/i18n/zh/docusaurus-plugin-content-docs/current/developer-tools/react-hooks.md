...
`bitsocial-react-hooks` 包提供了熟悉的 React hooks API，用于与 Bitsocial 协议交互。它可以处理获取提要、评论和作者个人资料、管理帐户、发布内容和订阅社区 - 所有这些都无需依赖中央服务器。

此库是 [5chan](/apps/5chan/) 和其他 Bitsocial 客户端应用程序使用的主要接口。

:::note
`bitsocial-react-hooks` 是为 AI 辅助维护的 `plebbit/plebbit-react-hooks` 的临时分支发展。它直接从 GitHub 使用，而不是发布到 npm。
:::

## 安装

因为该包尚未在 npm 上，所以直接从 GitHub 安装它，固定到特定的提交哈希：

```bash
yarn add https://github.com/bitsocialnet/bitsocial-react-hooks.git#<commit-hash>
```

将 `<commit-hash>` 替换为您想要定位的提交。

## API 概述

挂钩按功能类别进行组织。以下是每个类别中最常用的挂钩的摘要。有关完整的签名、参数和返回类型，请参阅 [GitHub 上的完整 API 参考](https://github.com/bitsocialnet/bitsocial-react-hooks).
...
- `useComment(commentCid?)` -- 通过 CID 获取单个评论
- `useComments(commentCids?)` -- 批量获取多个评论
