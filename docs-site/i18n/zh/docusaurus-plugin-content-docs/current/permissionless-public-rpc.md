---
title: 无许可公共 RPC
description: 针对具有隔离用户、范围权限和社区所有权的公共 Bitsocial RPC 服务的拟议设计。
---

# 无许可公共 RPC

最初的公共 RPC 提案以旧的 plebbit 术语编写的 GitHub 问题存在。本页面用 Bitsocial 语言重写了这一想法，并将其构建为产品级提案，而不是实施细节之墙。

## 简单语言目标

Bitsocial Forge 可以运行公共 RPC 服务，让许多用户远程管理自己的 Bitsocial 社区，而无需将运营商变成这些社区的托管人。

该服务应该使移动和轻量级客户端实用，同时保留三个约束：
...
今天，最简单的 RPC 模型通常是全有或全无：一个身份验证密钥、一个权限域、完全访问权限。这适用于单个操作员，但不适用于公共多用户服务。

无需许可的公共 RPC 需要更强大的模型：
...
每个用户都会获得一个身份验证凭据和一个权限包。

### 社区

通过该服务创建的社区会分配给一个所有者记录。显式跟踪所有权，以便将管理方法限定在正确的用户范围内。
...
公共 RPC 本身应该关注面向用户的 RPC 行为。用户创建、所有权转让和审核审核等管理任务属于单独的操作员 API 和仪表板。

## 兼容性立场

面向用户的文档应使用 **社区** 和 **个人资料** 等 Bitsocial 术语。

在线路级别，首次推出仍然可以保留当前的 ​​JSON-RPC 传输和有效负载形状，这对于兼容性很有用。换句话说：文档不再需要像旧的 plebbit 文档那样进行讨论，即使过渡期在幕后保留了一些遗留方法名称或请求形状。
...
权限感知应保持可选。忽略通知的客户端仍然可以通过处理来自服务器的标准授权失败来正确运行。

## 所有权强制执行

当服务创建社区时，它应该自动将所有权分配给调用用户。从这里开始：

- 社区启动、停止、编辑和删除操作是所有者范围的
- 列表和订阅响应默认为调用者自己的社区
- 更广泛的可见性是显式管理员权限，而不是默认
...
如果身份验证创建成本低廉，公共服务在颁发凭证之前可能需要挑战层。一种可能的途径是重用社区挑战模型本身，以便凭证颁发继承与网络其他部分相同的反滥用理念。
