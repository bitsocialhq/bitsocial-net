...
- **Mitigation:** Use `http://bitsocial.localhost:1355` first. Only bypass it with `PORTLESS=0 bun run dev` when you explicitly need a direct Vite port.
- **Status:** confirmed
...
- **Mitigation:** Use `git commit --no-verify -m "message"` for agent-created commits. Humans can still use `bun run commit` or `bunx cz`.
- **Status:** confirmed
