...
- Item: F002, F003
- Summary: Finished the local Grafana/Prometheus stack around the Bitsocial monitor: generated the summary and 5chan dashboards from the legacy public dashboard export, provisioned Grafana and Prometheus, added a shared container image for the monitor and IPFS sidecar, and verified that Grafana boots with the expected row structure and world map panel.
...
- Blockers: The imported protocol client stack pulls `skia-canvas`, so the shared monitor image needed `libfontconfig1` before the monitor could start inside Docker. Community fetch panels still reflect live network failures from the current gateways, but the dashboards themselves now render and scrape correctly.
