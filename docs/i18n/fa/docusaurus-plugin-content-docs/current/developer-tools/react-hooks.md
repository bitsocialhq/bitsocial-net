...
این کتابخانه رابط اصلی است که توسط [5chan](/apps/5chan/) و دیگر برنامه‌های سرویس گیرنده Bitsocial استفاده می‌شود.

:::note
`bitsocial-react-hooks` در حال حاضر مستقیماً از GitHub مصرف می شود نه اینکه در npm منتشر شود.
:::
...
`<commit-hash>` را با commitی که می خواهید هدف قرار دهید جایگزین کنید.

## نمای کلی API

قلاب ها به دسته های کاربردی سازماندهی می شوند. در زیر خلاصه ای از پرکاربردترین قلاب ها در هر دسته آورده شده است. برای امضاها، پارامترها و انواع بازگشت کامل، به [مرجع کامل API در GitHub] (https://github.com/bitsocialnet/bitsocial-react-hooks).) مراجعه کنید
...
- قلاب جستجوی یک جامعه -- یک انجمن را بر اساس آدرس واکشی می کند
- قلاب جستجوی چند جامعه -- چندین انجمن را واکشی می کند
- قلاب آمار انجمن -- تعداد مشترکین و پست ها را برمی گرداند
...
- قلاب تنظیمات RPC -- پیکربندی شبح RPC فعلی را برمی گرداند
