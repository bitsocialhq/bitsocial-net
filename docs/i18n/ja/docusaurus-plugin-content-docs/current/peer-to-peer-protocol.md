---
title: ピアツーピアプロトコル
description: Bitsocial が公開鍵アドレス指定とピアツーピアの pubsub を使用してサーバーレス ソーシャル メディアを配信する方法。
---
...
ブラウザはピアツーピア ネットワークに直接参加できません。 Bitsocial は、P2P ネットワークとブラウザ クライアントの間でデータを中継する **HTTP ゲートウェイ** でこれを処理します。これらのゲートウェイは次のとおりです。
...
これにより、ブラウザのエクスペリエンスがシームレスに保たれると同時に、その下の分散アーキテクチャが維持されます。
