...
merged_pr_number_for_branch() {
  ...
  if [ -n "$pr_number" ]; then
    gh pr view "$pr_number" --repo "$repo_slug" --json mergedAt --jq 'select(.mergedAt != null) | .mergedAt' >/dev/null 2>&1 || return 0
    echo "$pr_number"
    return 0
  fi

  gh pr list --repo "$repo_slug" --state merged --head "$branch" --json number --jq '.[0].number // empty' 2>/dev/null || true
}
